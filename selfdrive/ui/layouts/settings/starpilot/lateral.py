  ...
  def _build_panels(self):
    ...
    self._advanced_rows = [
      SettingRow(
        ...
        visible=lambda: alt_on() and cs.hasNNFFLog and not cs.isAngleCar,
      ),
      SettingRow(
        ...
        set_state=lambda s: (p.put_bool("NNFFLite", s),
                             _sync_parent(p, "LateralTune", _LATERAL_TUNE_KEYS)),
        visible=lambda: alt_on() and not cs.isAngleCar,
      ),
      SettingRow(
        ...
        set_state=lambda s: (p.put_bool("ForceTorqueController", s),
                             _sync_parent(p, "LateralTune", _LATERAL_TUNE_KEYS)),
        visible=lambda: alt_on() and not cs.isTorqueCar and not cs.isAngleCar,
      ),
      SettingRow(
      ...
      SettingRow(
        ...
        visible=lambda: alt_on() and not cs.hasAutoTune and cs.isTorqueCar and not cs.isAngleCar,
      ),
      SettingRow(
        ...
        set_state=lambda s: (p.put_bool("ForceAutoTuneOff", s),
                             ...
                             _sync_parent(p, "AdvancedLateralTune", _ADVANCED_LATERAL_KEYS)),
        visible=lambda: alt_on() and cs.isTorqueCar and not cs.isAngleCar,
      ),
      SettingRow(
