...
class NRDRTuningLayout(_SettingsPage):
  ...
  def __init__(self):
    ...
    filter_rows = [
      ...
      value(
        "HondaTorqueOutputLpfTauLowSpeed", "Output LPF Tau: Low Speed", "Torque-output low-pass time constant below 25 mph.",
        lambda: f"{p.get_float('HondaTorqueOutputLpfTauLowSpeed'):.2f}",
        lambda: self._show_slider("HondaTorqueOutputLpfTauLowSpeed", 0.0, 5.0, step=0.01, value_type="float", title="Output LPF Tau: Low Speed"),
        visible=lambda: p.get_bool("HondaTorqueOutputLowPassFilter"),
      ),
      value(
        "HondaTorqueOutputLpfTauStandard", "Output LPF Tau: Standard", "Torque-output low-pass time constant from 25 to 50 mph.",
        lambda: f"{p.get_float('HondaTorqueOutputLpfTauStandard'):.2f}",
        lambda: self._show_slider("HondaTorqueOutputLpfTauStandard", 0.0, 5.0, step=0.01, value_type="float", title="Output LPF Tau: Standard"),
        visible=lambda: p.get_bool("HondaTorqueOutputLowPassFilter"),
      ),
      value(
        "HondaTorqueOutputLpfTauHighway", "Output LPF Tau: Highway", "Torque-output low-pass time constant above 50 mph.",
        lambda: f"{p.get_float('HondaTorqueOutputLpfTauHighway'):.2f}",
        lambda: self._show_slider("HondaTorqueOutputLpfTauHighway", 0.0, 5.0, step=0.01, value_type="float", title="Output LPF Tau: Highway"),
        visible=lambda: p.get_bool("HondaTorqueOutputLowPassFilter"),
      ),
