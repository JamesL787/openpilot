...
from starpilot.common.testing_grounds import testing_ground

# Compatibility aliases for branch-local GM enum names.
GM_BOLT_ACC_2022_2023 = getattr(GM_CAR, "CHEVROLET_BOLT_ACC_2022_2023", GM_CAR.CHEVROLET_BOLT_EUV)
GM_BOLT_ACC_2022_2023_PEDAL = getattr(GM_CAR, "CHEVROLET_BOLT_ACC_2022_2023_PEDAL", GM_CAR.CHEVROLET_BOLT_EUV)
GM_BOLT_CC_2022_2023 = getattr(GM_CAR, "CHEVROLET_BOLT_CC_2022_2023", GM_CAR.CHEVROLET_BOLT_NON_ACC_2ND_GEN)
GM_BOLT_CC_2018_2021 = getattr(GM_CAR, "CHEVROLET_BOLT_CC_2018_2021", GM_CAR.CHEVROLET_BOLT_NON_ACC_1ST_GEN)
GM_BOLT_CC_2017 = getattr(GM_CAR, "CHEVROLET_BOLT_CC_2017", GM_CAR.CHEVROLET_BOLT_NON_ACC)
GM_VOLT_ASCM = getattr(GM_CAR, "CHEVROLET_VOLT_ASCM", GM_CAR.CHEVROLET_VOLT)
GM_VOLT_CAMERA = getattr(GM_CAR, "CHEVROLET_VOLT_CAMERA", GM_CAR.CHEVROLET_VOLT)
GM_VOLT_CC = getattr(GM_CAR, "CHEVROLET_VOLT_CC", GM_CAR.CHEVROLET_VOLT)
...
BOLT_2022_2023_CARS = (
  GM_BOLT_ACC_2022_2023,
  GM_BOLT_ACC_2022_2023_PEDAL,
  GM_BOLT_CC_2022_2023,
)
BOLT_2018_2021_CARS = (
  GM_BOLT_CC_2018_2021,
)
BOLT_2017_CARS = (
  GM_BOLT_CC_2017,
)
...
def get_clarity_nidec_ff_scale(
  desired_lateral_accel: float,
  desired_lateral_jerk: float,
  v_ego: float,
  firestar_active: bool = False,
) -> float:
  if desired_lateral_accel == 0.0:
    return 1.0
  ...
  base_reduction = _clarity_nidec_side_value(
    desired_lateral_accel,
    CLARITY_NIDEC_FF_REDUCTION_LEFT,
    CLARITY_NIDEC_FF_REDUCTION_RIGHT,
  ) * onset * cutoff
  ...
  installed_tune_active = not firestar_active
  if installed_tune_active:
    base_reduction -= _clarity_nidec_side_value(
      desired_lateral_accel,
      CLARITY_NIDEC_A_VARIANT_FF_RESTORE_LEFT,
      CLARITY_NIDEC_A_VARIANT_FF_RESTORE_RIGHT,
    ) * onset * cutoff
  if firestar_active:
    base_reduction += _clarity_nidec_side_value(
      desired_lateral_accel,
      CLARITY_NIDEC_B_VARIANT_FF_REDUCTION_LEFT,
      CLARITY_NIDEC_B_VARIANT_FF_REDUCTION_RIGHT,
    ) * onset * cutoff

  turn_in_boost = 1.0 + (
    _clarity_nidec_side_value(
      desired_lateral_accel,
...
def get_clarity_nidec_friction_scale(
  v_ego: float,
  desired_lateral_accel: float,
  desired_lateral_jerk: float,
  firestar_active: bool = False,
...
class LatControlTorque(LatControl):
  def __init__(self, CP, CP_SP, CI, dt, honda_firestar=False):
    super().__init__(CP, CP_SP, CI, dt)
    self.CP_SP = CP_SP
    # The Honda Firestar GUI flag maps the historic Clarity 5B tune onto this branch.
    self.honda_firestar = bool(honda_firestar)
    ...
    self.is_clarity_nidec = CP.carFingerprint == HONDA_CAR.HONDA_CLARITY and bool(getattr(CP_SP, "flags", 0) & HondaFlagsSP.EPS_MODIFIED.value)
    self.is_volt_cc = CP.carFingerprint == GM_VOLT_CC
  ...
  def update(self, active, CS, VM, params, steer_limited_by_safety, desired_curvature, calibrated_pose, curvature_limited, lat_delay=None):
    ...
    if not active:
      ...
      clarity_nidec_firestar_active = self.honda_firestar if clarity_nidec_active else False
      ...
      if bolt_2022_2023_tuned_path_active:
        ...
        ff *= get_clarity_nidec_ff_scale(setpoint, desired_lateral_jerk, CS.vEgo, clarity_nidec_firestar_active)
        friction_threshold = CLARITY_NIDEC_FIXED_FRICTION_THRESHOLD
        friction_scale = get_clarity_nidec_friction_scale(CS.vEgo, setpoint, desired_lateral_jerk, clarity_nidec_firestar_active)
