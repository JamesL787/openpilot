...
class ModelState(ModelStateBase):
  ...
  def _init_combined(self, pkl_path, cam_w, cam_h, bundle):
    ...
    camera_jits = jits[(cam_w, cam_h)]
    self._separate_warp_policy = not isinstance(camera_jits, dict)
    if self._separate_warp_policy:
      self._run_policy = jits['run_policy']
      self._warp_enqueue = camera_jits
    else:
      self._run_policy = camera_jits['run_policy']
      self._warp_enqueue = camera_jits['warp_enqueue']
    if self._separate_warp_policy and 'action_t' in self._run_policy.captured.expected_names and 'action_t' not in self.input_queues:
      action_t_shape = self.npy.get('traffic_convention', np.zeros((1, 2), dtype=np.float32)).shape
      self.npy['action_t'] = np.zeros(action_t_shape, dtype=np.float32)
      self.input_queues['action_t'] = Tensor(self.npy['action_t'], device='NPY').realize()
    road_name = next(k for k in self._vision_input_names if 'big' not in k)
    yuv_size = self.frame_buf_params[road_name][3]
    warmup_inputs = self.input_queues
    if self._separate_warp_policy:
      warmup_inputs = {k: self.input_queues[k] for k in ('img_q', 'big_img_q', 'tfm', 'big_tfm') if k in self.input_queues}
    self._warp_enqueue(
      **warmup_inputs,
  ...
  def run(self, bufs: dict[str, VisionBuf], transforms: dict[str, np.ndarray],
    ...
    if self._separate_warp_policy:
      warp_inputs = {k: self.input_queues[k] for k in ('img_q', 'big_img_q', 'tfm', 'big_tfm') if k in self.input_queues}
      img, big_img = self._warp_enqueue(**warp_inputs, frame=self.full_frames[road_key], big_frame=self.full_frames[wide_key])
      if prepare_only:
        return None
      policy_inputs = {k: self.input_queues[k] for k in ('feat_q', 'desire_q', 'desire', 'traffic_convention', 'action_t') if k in self.input_queues}
      raw_outputs = self._run_policy(**policy_inputs, img=img, big_img=big_img)
    elif prepare_only:
